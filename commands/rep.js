...
module.exports = {
  ...
  execute(msg, args) {
    ...
    db.get('SELECT user_ratingDate FROM users WHERE user_id=?', [msg.author.id], (err, row) => {
      let today = new Date()
      ...
      if (today < ratingDate) {
        ...
        db.get('SELECT user_rating, username FROM users WHERE user_id=?', [userID], (err, row) => {
          ...
          db.run('UPDATE users SET user_ratingDate=? WHERE user_id=?', [String(newDate), msg.author.id])
        })
