    ...
    db.serialize(() => {
      db.get(
        ...
        async (err, row) => {
          ...
          if (row !== undefined) {
            let today = new Date();
            let daysOnServer = Math.ceil(
              Math.abs(
                today.getTime() - new Date(row.user_joinedTimestamp).getTime()
              ) /
                (1000 * 3600 * 24)
            );

            let response = await fetch(
              row.user_avatarURL.replace(".webp", ".png")
            );
            ...
            if (row.user_rating === 0)
              iconSmile = await loadImage("./icons/mdi_emoticon-neutral.png");
            else if (row.user_rating > 0 && row.user_rating < 25)
              iconSmile = await loadImage("./icons/mdi_emoticon-happy.png");
            else if (row.user_rating >= 25)
              iconSmile = await loadImage("./icons/mdi_emoticon-excited.png");
            else if (row.user_rating < 0 && row.user_rating > -25)
              iconSmile = await loadImage("./icons/mdi_emoticon-sad.png");
            else if (row.user_rating <= -25)
              iconSmile = await loadImage("./icons/mdi_emoticon-devil.png");

            let nextLevel = 500 * (Math.pow(2, row.user_level) - 1);
            let progress = Math.round(
              ((100 / (nextLevel / row.user_xp)) * 280) / 100
            );
            ...
            let result = new Canvas(450, 150)
              ...
              .printText(
                `${row.user_xp} / ${nextLevel}`,
                userInfoX + progressBarW / 2,
                progressTextSize + progressBarY + 1
              )
              .toBuffer();
